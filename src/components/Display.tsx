...
const Display: React.FC<DisplayProps> = ({ value, error }) => {
  return (
    <div className="relative w-full h-24 rounded-xl mb-4 overflow-hidden bg-gray-900">
      {/* アニメーションするドットパターン */}
      ...
      {/* コンテンツ */}
      <div className="relative z-10 h-full text-white text-right p-4 transition-all duration-300">
        {error ? (
          <div className="text-red-500 text-sm mb-1">{error}</div>
        ) : null}
        <div className={`${orbitron.className} text-4xl overflow-x-auto whitespace-nowrap scrollbar-hide tracking-wider`}>
          {value || '0'}
        </div>
      </div>
    </div>
